...
int zipentry_open(lua_State* L)
{
  ...
  int ignorecase = (int) luaL_optinteger(L, 3, 0);
  p_zipentry zent;
  int err;
...
int luaopen_libunzip(lua_State *L)
{
  /* Register the functions and tables */
  luaL_newmetatable(L, "LuaZ:UnZip");
  luaL_newlib(L,zipfile_meta);

  lua_newtable(L);
  luaL_newlib(L,zipfile_methods);
  lua_setfield(L, -2, "__index");
  lua_pop(L, 1);

  /* Register the functions and tables */
  luaL_newmetatable(L, "LuaZ:UnZip:Entry");
  luaL_newlib(L,zipentry_meta);

  lua_newtable(L);
  luaL_newlib(L,zipentry_methods);
  lua_setfield(L, -2, "__index");
  lua_pop(L, 1);
